...
- ⚠️ 빠져나갈 길이 없는 화면(막다른 길) 반복 → 화면 패킷 프롬프트에 '모든 화면에 돌아가거나 진행할 경로를 둘 것 — 뒤로가기·홈·다음 중 최소 하나' 경고 추가 (신뢰도 95%, 163회 관찰)
...
- ⚠️ 화면 이동 후 입력·선택이 사라짐 반복 → 패킷 프롬프트에 '사용자 입력은 화면 이동을 넘어 유지할 것 — 라우트 전환 시 초기화되는 지역 상태에 담지 말 것' 경고 추가 (신뢰도 91%, 41회 관찰)
